...
class CustomTokenObtainPairViewTestStripe(SkipStripeSignalsTestCase, APITestCase, URLPatternsTestCase):
    urlpatterns = [
...
class CustomTokenRefreshViewTestStripe(SkipStripeSignalsTestCase, APITestCase, URLPatternsTestCase):
    urlpatterns = [
        path("api/auth/", include("apps.authentication.urls")),
    ]
...
class ChangeUserEmailTestStripe(SkipStripeSignalsTestCase, APITestCase, URLPatternsTestCase):
    urlpatterns = [
        path("api/auth/", include("apps.authentication.urls")),
    ]
...
class SendConfirmationEmailTestStripe(SkipStripeSignalsTestCase, APITestCase, URLPatternsTestCase):
    urlpatterns = [
        path("api/auth/", include("apps.authentication.urls")),
    ]
...
class ForgotPasswordViewTestStripe(SkipStripeSignalsTestCase, APITestCase, URLPatternsTestCase):
    urlpatterns = [
        path("api/auth/", include("apps.authentication.urls")),
    ]
    ...
    @staticmethod
    def __send_post_request_forgot_password(data):
        api_request = APIRequestFactory().post("", data=data)
    ...
    @staticmethod
    def __send_get_request_forgot_password(data):
    ...
    @staticmethod
    def __send_get_request_forgot_password_with_auth(data, auth_user):
        api_request = APIRequestFactory().get("", data)
        detail_view = ForgotPasswordView.as_view()
...
class TempEmailGeneratingTestStripe(SkipStripeSignalsTestCase):
    def setUp(self):
        super().setUp()
    ...
    def test_generating(self):
        ...
        self.assertTrue(timestamp < timezone.now().timestamp())


class SocialAccountDeleteTestStripe(SkipStripeSignalsTestCase, APITestCase, URLPatternsTestCase):
    urlpatterns = [
        path("api/auth/", include("apps.authentication.urls")),
    ]
    ...
    def setUp(self):
        ...
        self.email = "vitalik_t@dataforest.ai"
        self.password = "12351994"
        self.user = baker.make("user.User") if not hasattr(self, "user") else getattr(self, "user")
        self.user.email = self.email
