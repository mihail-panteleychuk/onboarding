...
class BaseAdminPanelUserSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = ('id', 'avatar', 'first_name', 'last_name', 'email', 'role', 'is_active', 'is_deleted', 'onboarding_finished', 'password')
...
class AdminPanelUserCreateSerializer(BaseAdminPanelUserSerializer):
    password = serializers.CharField(write_only=True, required=False, allow_null=True, validators=[validate_user_password])
