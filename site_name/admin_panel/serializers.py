...
from user.validators import role_validator, validate_user_password
...
class AdminPanelUserUpdateSerializer(BaseAdminPanelUserSerializer):
    role = serializers.JSONField(
        source="get_role_object", required=False, validators=[role_validator]
    )


class AdminPanelUserCreateSerializer(BaseAdminPanelUserSerializer):
    password = serializers.CharField(
        write_only=True, required=False, allow_null=True, validators=[validate_user_password]
    )
    role = serializers.JSONField(
        source="get_role_object", default=get_user_model().USER, validators=[role_validator]
    )
