...
class CustomBlacklistMixin(BlacklistMixin):
    def check_blacklist(self):
        ...
        if BlacklistedToken.objects.filter(token__jti=jti).exists():
            raise TokenError(_('You have been logged out since you just signed into Dropship from another location.'))
...
class CustomJWTAuthentication(JWTAuthentication):
    ...
    def check_user_email(self, user):
        """
            ...
            Grant access only for `dropship.io` and `dataforest.ai`
        """
        ...
        for pattern_type, pattern in dict(white_list_patterns).items():
            ...
            raise PermissionDenied(_('Your email is not permitted to authorize on the STAGE environment.'), code='permission_denied')
