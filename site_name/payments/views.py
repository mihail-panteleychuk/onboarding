...
from re import sub
from core.exceptions import APIException, CustomAPIException
...
class CreateCardView(generics.CreateAPIView):
    ...
    def __attach_payment_method(self, payment_id):
        ...
        try:
            ...
            raise CustomAPIException(detail={'detail': e.error.message}, code=status.HTTP_409_CONFLICT)
    ...
    def __create_billing_address(self, card, request_data):
        address = StripeCardBillingSerializer(
    ...
    def post(self, request, *args, **kwargs):
        ...
        serializer = CreateCardViewSerializer(data=request.data)
        ...
        if self.request.user.cards.filter(payment_method_id=data['payment_method']['id']).exists():
            return Response({'message': {'detail': 'Payment method already exists'}}, status=status.HTTP_409_CONFLICT)

        # customer = retreive_customer(self.request.user)
...
class CreateSubscriptionView(CreateCardView):
    ...
    def post(self, request, *args, **kwargs):
        serializer = CreateSubscriptionViewSerializer(data=request.data, context={'request': self.request})
        if not serializer.is_valid():
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

        data = serializer.data
        if data.get('payment_method'):
            resp = super(CreateSubscriptionView, self).post(request, *args, **kwargs)
            if resp.status_code != status.HTTP_201_CREATED and resp.data['message']['detail'] != 'Payment method already exists':
                return resp
        redirect_url = data.pop('redirect_url', None)
        resp = create_subscription_for_user(self.request.user, data['plan'], coupon=data['discount'], redirect_url=redirect_url)
        ...
        if redirect_link:
            return Response({'paid': False, 'link': redirect_link}, status=status.HTTP_402_PAYMENT_REQUIRED)

        return Response({'user': self.get_serializer(self.request.user).data, 'paid': True},
...
class UpdateSubscriptionView(generics.CreateAPIView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if isinstance(old_schedule, (str, type(None)) ):
            return Response({'user': self.get_serializer(self.request.user).data,
    ...
    def post(self, request, *args, **kwargs):
        serializer = UpdateSubscriptionViewSerializer(data=request.data, context={'request': self.request})
        if not serializer.is_valid():
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

        data = serializer.data
        redirect_url = data.pop('redirect_url', None)
        data['plan'] = get_object_or_404(Plan.objects.all(), pk=data['plan']) if data['plan'] else None
        if data['force']:
            redirect_link  = update_stripe_subscription(self.request.user, data['plan'], coupon=data['discount'], redirect_url=redirect_url)
            if redirect_link:
                return Response({'paid': False, 'link': redirect_link}, status=status.HTTP_402_PAYMENT_REQUIRED)
        else:
...
class InvoiceViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_serializer_class(self):
        if self.action == 'retrieve':
            self.serializer_class = DetailedInvoiceSerializer
...
class StripeWebhookView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        webhook_secret = 'whsec_vAFt71Du1k7XUcfTVsqDBKJnsdxk2oEf'
        ...
        if event_type in ['invoice.finalized', 'invoice.payment_succeeded', 'invoice.paid', 'invoice.voided']:
            ...
            subscription =Subscription.objects.filter(
                ...
                status__in=ACTIVE_STATUSES if event_type not in  ['invoice.voided', 'invoice.payment_failed'] else ALL_STATUSES).latest('created')

            # write down an invoice
            write_invoice(stripe_subscription,
        ...
        if event_type == 'invoice.payment_failed':
            if data['object']['billing_reason'] == 'subscription_create':
                ...
                if data['object']['attempt_count'] > 0:
                    ...
                    res = stripe.Invoice.void_invoice(
                        latest_invoice
                    )


        if event['type'] == 'customer.subscription.updated' and event['data'].get('previous_attributes'):
            ...
            #         latest_invoice,
            #     )
            if previous_attributes.get('plan') and previous_attributes['plan']['id'] != subscription['plan']['id']:
                user_subscription = get_object_or_404(
                    ...
                    # scheduled_plan__payment_plan_id=subscription['plan']['id'],

                user = user_subscription.user
                expire_date = subscription.get('current_period_start', now().timestamp())
                ...
                user_subscription.update_obj({
                    ...
                    'next_payment_date': datetime.fromtimestamp(subscription['current_period_end'], tz=timezone.utc),
                    'expire_date': None,
        ...
        if event['type'] == 'subscription_schedule.canceled':
            ...
            user_subscription = get_object_or_404(
                ...
                user__payment_service_user_id=subscription['customer'],
                status=STATUS_PAID_CANCELED
            )
            user_subscription.update_obj(
                {
                    'status': get_status(subscription['status']),
                    'expire_date': datetime.fromtimestamp(subscription.get('canceled_at', int(now().timestamp())), tz=timezone.utc)
                }
            )
            user_subscription.save()
