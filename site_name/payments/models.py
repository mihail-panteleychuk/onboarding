...
# from subscription.constants import PAYMENT_STATUSES, STATUS_FAILED
from core.models import BaseUuidModel
from subscription.models import Subscription, PAYMENT_STATUSES, STATUS_FAILED

class Card(BaseUuidModel):
    brand = models.CharField(max_length=128)
    country = models.ForeignKey('user.Country', related_name='cards', on_delete=models.SET_NULL, null=True, blank=True)
    exp_month = models.IntegerField()
    exp_year = models.IntegerField()
    last4 = models.CharField(max_length=4, null=False, blank=False)

    user = models.ForeignKey('user.User', related_name='cards', on_delete=models.CASCADE, db_index=True)
...
class BillingAddress(BaseUuidModel):
    card = models.OneToOneField('payments.Card', on_delete=models.CASCADE, related_name='card_billing')
...
class InvoiceLineItem(BaseUuidModel):
    subscription = models.ForeignKey('subscription.Subscription', related_name='invoice_lines', on_delete=models.SET_NULL, null=True, blank=True)
    invoice = models.ForeignKey('payments.Invoice', on_delete=models.CASCADE, related_name='line_items')
...
class Invoice(models.Model):
    ...
    credits_applied = models.IntegerField(default=0)  # sum of applied credits from existing subscription (force upgrade)
    ...
    status = models.PositiveSmallIntegerField(choices=PAYMENT_STATUSES, default=STATUS_FAILED)
    ...
    def get_status_object(self):
        if self.status or isinstance(self.status, int):
            return {
                'id': self.status,
                'name': dict(PAYMENT_STATUSES)[self.status]
            }
        return None
