...
from .models import BillingAddress, Card, Invoice, InvoiceLineItem
...
class CreateSubscriptionViewSerializer(CreateCardViewSerializer):
    ...
    class Meta:
        fields = ('payment_method', 'first_name', 'last_name', 'country', 'state', 'type', 'plan', 'discount', 'redirect_url')
...
class DetailedInvoiceSerializer(serializers.ModelSerializer):
    ...
    def get_bill_from(self, obj):
        from .utils import retrieve_stripe_account_billing
    ...
    class Meta:
        model = Invoice
        fields = ('invoice_id', 'status', 'bill_from', 'line_items', 'card', 'issued', 'credits_applied', 'discount', 'total')
