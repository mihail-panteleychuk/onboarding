...
class CustomTokenObtainPairSerializer(TokenObtainSerializer):
    ...
    def validate(self, attrs):
        ...
        data["user"] = FullUserInfoSerializer(
            self.user, context={"request": self.context.get("request")}
        ).data
        return data
    ...
    def validate(self, data: dict[str, str]):
        if not data["email"]:
            raise serializers.ValidationError(
                self.default_error_messages["old_email_empty"], code=HTTP_400_BAD_REQUEST
            )
        if not data["new_email"]:
            raise serializers.ValidationError(
                self.default_error_messages["new_email_empty"], code=HTTP_400_BAD_REQUEST
            )
        if User.objects.filter(email=data["new_email"].lower()).exists():
            raise serializers.ValidationError(
                self.default_error_messages["new_email_exists"], code=HTTP_400_BAD_REQUEST
            )
        self.instance = get_object_or_404(User.objects.all(), email=data["email"].lower())

        if self.instance.email_confirmed:
            raise serializers.ValidationError(
                self.default_error_messages["old_email_verified"], code=HTTP_400_BAD_REQUEST
            )
