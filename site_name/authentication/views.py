...
from .exceptions import (
    ACCOUNT_IS_NOT_ACTIVE_EXCEPTION,
    BLOCKED_ACCOUNT_EXCEPTION,
    CANNOT_DETACH_LAST_SOCIAL_FROM_ACCOUNT_EXCEPTION,
    CODE_EXPIRED_OR_INVALID_EXCEPTION,
    INVALID_EMAIL_EXCEPTION,
    INVALID_USER_DATA_EXCEPTION,
)
        ...
        if self.request.user.is_authenticated:  # check authenticated request
            if user_with_social and user_with_social != self.request.user:
                ...
                raise CustomAPIException(
                    detail={"detail": error_already_connected}, code=HTTP_400_BAD_REQUEST
                )
            ...
            if user_with_social:
                if user_with_social.is_deleted:
                    ...
                    raise BLOCKED_ACCOUNT_EXCEPTION
                elif not user_with_social.is_active:
                    # check if user is existing
                    raise ACCOUNT_IS_NOT_ACTIVE_EXCEPTION
                user = user_with_social
            elif temp_u:
                # user is trying to sign in with social account after he already
                ...
                if temp_u.is_deleted:
                    ...
                    raise ACCOUNT_IS_NOT_ACTIVE_EXCEPTION
                raise CustomAPIException(
                    detail={"detail": error_disconnected}, code=HTTP_400_BAD_REQUEST
                )
        return user
...
class AppleTokenView(APIView, BaseAuthView):
    ...
    def delete(self, request):
        if request.user.is_anonymous:
            return Response(
                {"message": {"detail": _("Not Authenticated")}}, status=HTTP_401_UNAUTHORIZED
            )
        ...
        if (
            ...
            raise CANNOT_DETACH_LAST_SOCIAL_FROM_ACCOUNT_EXCEPTION
...
class SendConfirmationEmail(APIView, BaseAuthView):
    ...
    def get(self, request):
        ...
        if not all([code, confirm_code]):
            raise CODE_EXPIRED_OR_INVALID_EXCEPTION
    ...
    def post(self, request):
        ...
        if not self.request.user.is_anonymous:
            ...
            if not all([first_name, last_name, email]):
                raise INVALID_USER_DATA_EXCEPTION
            ...
            if user and user.is_active:
                ...
                user = User(
                    first_name=first_name, last_name=last_name, email=email, is_active=False
                )
