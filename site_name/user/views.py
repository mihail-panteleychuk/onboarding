...
from .serializers import ChangeEmailSerializer, ChangePasswordSerializer, FullUserInfoSerializer
...
class ChangeUserEmail(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        confirm_code = ConfirmCode.objects.filter(
            user=user, new_email=data["new_email"].lower()
        ).first()
        if not confirm_code:
        ...
        server.quit()
        return Response(
            {"message": {"detail": _("Confirmation email was sent")}}, status=status.HTTP_200_OK
        )
    ...
    def get(self, request):
        ...
        confirm_code = ConfirmCode.objects.filter(
            code=code, expiring_date__gt=now(), new_email__isnull=False
        ).first()
