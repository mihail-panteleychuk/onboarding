...
from site_name.user.filters import UserFilter
from site_name.user.models import ConfirmCode, Country, User
from site_name.user.serializers import (
...
from site_name.user.utils import detect_country_by_ip
...
class UserViewSet(viewsets.GenericViewSet, mixins.UpdateModelMixin):
    ...
    def change_password(self, request):
        ...
        if serializer.is_valid():
            ...
            if user.check_password(request.data.get("new_password")):
                response = {
                    "message": {
                        "detail": _("You already have this password"),
                        "type": "new_password",
                    },
                }
            ...
            return Response(
                {
                    "message": {
                        "detail": _("Your password has successfully changed!"),
                        "type": "success",
                    },
                },
    ...
    def set_password(self, request):
        ...
        if not user.is_active:
            return Response(
                {"message": {"detail": _("User email is not confirmed.")}, "status_code": 400},
        ...
        # updating user_info as `onboarding_finished=True` if
        # onboarding ending with password setting
        # (without subscriptions creating)
        if settings.ONBOARDING_ENABLED:
            user.onboarding_finished = user.onboarding_finished or not settings.ONBOARDING_ENABLED
        else:
        ...
        if serializer.is_valid():
            ...
            if user.check_password(data.get("new_password")):
                response = {"password": [_("You already have this password")]}
                return Response(response, status=status.HTTP_400_BAD_REQUEST)
            ...
            user.is_active = True
            user.save()
            code = ConfirmCode.objects.filter(
                user=user,
                expiring_date__gt=now(),
                new_email__isnull=True,
                new_password__isnull=True,
            ).first()
            if code:
                code.delete()
            ...
            return Response(
                {
                    "message": {"detail": _("A password has been successfully set!")},
                    "onboarding_finished": user.onboarding_finished,
                },
                status=status.HTTP_200_OK,
            )
    ...
    def set_language(self, request, *args, **kwargs):
        ...
        if not lang:
            return Response(
                {"message": {"detail": _("Required field 'language' is not passed.")}},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class CountryListView(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        queryset = self.queryset.extra(select={"int_code": "CAST(code AS INTEGER)"}).order_by(
            "int_code",
            "name",
        )
        return queryset
...
class ChangeUserEmail(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        server.quit()
        return Response(
            {"message": {"detail": _("Confirmation email was sent")}},
            status=status.HTTP_200_OK,
        )
