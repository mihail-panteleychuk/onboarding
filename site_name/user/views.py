...
class UserViewSet(viewsets.GenericViewSet, mixins.UpdateModelMixin):
    ...
    def change_password(self, request):
        ...
        if serializer.is_valid():
            ...
            if not user.check_password(request.data.get("old_password")):
                response = {"message": {"detail": _("Current password is entered incorrectly"), 'type': "current_password"}}
            ...
            return Response({"message": {"detail": _("Your password has successfully changed!"), 'type': "success"}}, status=status.HTTP_200_OK)
    ...
    def set_password(self, request):
        ...
        # updating user_info as `onboarding_finished=True` if onboarding ending with password setting
        # (without subscriptions creating)
        ...
        if serializer.is_valid():
            ...
            user.is_active = True
            user.save()
            code = ConfirmCode.objects.filter(user=user, expiring_date__gt=now(), new_email__isnull=True, new_password__isnull=True).first()
            if code: code.delete()
    ...
    @action(detail=False, methods=['POST'], url_path='set-language', permission_classes=[permissions.IsAuthenticated],
            serializer_class=AccountBillingSerializers)
    def set_language(self, request, *args, **kwargs):
        """ Update user selected languages """
        lang = request.data.get('language')
        if not lang:
            return Response({"message": {"detail": _("Required field 'language' is not passed.")}}, status=status.HTTP_400_BAD_REQUEST)

        user = request.user
        user.language = lang
        user.save()
        resp = self.get_serializer(user).data

        return Response(resp, status=status.HTTP_200_OK)

    @action(detail=False, methods=['get'], url_path='get-language', permission_classes=[permissions.IsAuthenticated],
            serializer_class=LanguageSerializer)
    def list_languages(self, request, *args, **kwargs):
        """ Retrieve list of available languages """
        response = LanguageSerializer(settings.LANGUAGES, many=True).data
...
class CountryListView(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        queryset = self.queryset.extra(
            select={'int_code': 'CAST(code AS INTEGER)'}
        ).order_by('int_code', 'name')
        return queryset
...
class ChangeUserEmail(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # change link
        link = f'https://{settings.FRONT_DOMAIN}/setting/{confirm_code.code}'
        ...
        _from = f"DataForest.io <{settings.EMAIL_HOST_USER}>"
        msg['From'] = _from
...
class DetectRequestIP(generics.RetrieveAPIView):
    permission_classes = (IsAuthenticated,)
    ...
    def get(self, request, *args, **kwargs):
        try:
            country = detect_country_by_ip(request)
            country = self.get_serializer(country)
            return Response(country.data, status=status.HTTP_200_OK)
        except:
