...
class UserViewSetTest(APITestCase, URLPatternsTestCase):
    ...
    def test_update_user_name(self):
        ...
        api_request = APIRequestFactory().patch(
            "", data={"first_name": new_name}, format=None, content_type=None
        )
    ...
    def test_update_role_invalid(self):
        ...
        new_role = 20
        api_request = APIRequestFactory().patch(
            "", data={"role": new_role}, format=None, content_type=None
        )
        detail_view = UserViewSet.as_view({"patch": "update_user", "put": "update_user"})
        force_authenticate(api_request, user=self.user_model)
        resp = detail_view(api_request)
    ...
    def test_set_language(self):
        ...
        self.assertIsInstance(resp.data, dict)
        self.assertEqual(
            resp.data["message"]["detail"], "Required field 'language' is not passed."
        )
...
class ChangeUserEmailViewTest(APITestCase, URLPatternsTestCase):
    ...
    def test_change_email_to_current_email_address_already_exists(self):
        data = {
            "password": self.password,
        ...
        self.assertIsInstance(resp.data, dict)
        self.assertEqual(
            resp.data["message"]["detail"], "This email already exists. Kindly try another one"
        )
    ...
    def test_confirm_code_from_email(self):
        ...
        code_obj = ConfirmCode.objects.filter(
            expiring_date__gt=now(), new_email__isnull=False, user=self.user
        ).first()
        query = {
            "code": code_obj.code,
        }
