...
def update_translates_back(request):
    pathes = [
        ...
        {"lang": "zh-cn", "path": settings.BASE_DIR / 'locale' / 'zh-hans' / 'LC_MESSAGES'/ 'django.po'},
    ]
    def read_gsheet():
        path = settings.BASE_DIR / 'token.json'
        gc = gspread.service_account(filename=path)
        sh = gc.open_by_key('1LlTYk2uP4_psKWNgTq4r0bfYWEl2xB5GBEokZdzA1t4').worksheet('Backend')
    ...
    for po_file in pathes:
        try:
            ...
            print(po_file['path'])
            print(e)
            return Response({"message": {"detail":"Error while exporting translates", 'error': str(e)}}, status = status.HTTP_400_BAD_REQUEST)
