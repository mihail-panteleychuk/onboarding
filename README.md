...
:attention: **Refactoring InProgress** - some following instructions may be out of date.
...
* check full list in [pyproject.toml](web_app/pyproject.toml)
...
1.  Rename folders from `web_app` to necessary name

1.  Rename project name in [pyproject.toml](web_app/pyproject.toml)
...
1.  Installing all listed in [pyproject.toml](web_app/pyproject.toml) dependencies
```bash
    make install
```
